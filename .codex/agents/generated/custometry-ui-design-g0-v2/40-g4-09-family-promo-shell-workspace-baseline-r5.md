...
gate_id: G4
target_id: family.promo.shell-workspace.baseline-r5
...
Execute only `G4@family.promo.shell-workspace.baseline-r5` after bounded ledger context and an atomic claim. Use the current active contract, preserve historical evidence and foreign changes, satisfy the exact gate and adjacent-stage preflight, then durably update the sole stage ledger. Do not claim another row across an owner checkpoint.
