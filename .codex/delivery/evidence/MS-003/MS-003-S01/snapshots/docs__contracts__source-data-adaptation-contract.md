...
## MS-003/S01 bounded implementation

The additive `retail-report/v1` runner profile loads Customer, Product, Receipt,
ReceiptItem, Store and Calendar through the existing PostgreSQL readonly adapter.
The default `retail/v1` profile remains selected unless explicitly overridden.
No new source wizard, general remediation engine or historical refresh acceptance
is implied. The accepted source is the unchanged `demo` seed `20260716`.

Six raw Parquet artifacts retain every source row. `retail-report-orphan-product/v1`
quarantines exactly the five known absent-product sentinel rows (`999999`) into a
protected diagnostic artifact, retaining their raw artifact/key provenance and
`receipt_item.product.reference` reason. Unknown or null product references,
mandatory null/duplicate keys and broken customer/receipt/store/calendar references
block admission. No waiver or fabricated Product is used. Canonical keys and
qualified references are rechecked after remediation.

Six canonical artifacts bind all entities into one semantic version. All source IDs
are lossless strings under `northwind-retail`; source connection UUID remains a
separate catalog identity. Receipt key is `[source_system_id, receipt_id]`; ReceiptItem
uses `[source_system_id, receipt_id, line_id]` from the stable source item ID.
Customer/Product/Store have deterministic version IDs and qualified natural keys.
`retail-report-current-only/v1` declares five one-to-many relationships, including
nullable Customer linkage for anonymous receipts and UTC receipt-date linkage to
Calendar. Supplied validity is retained; Store validity records the batch observation
rather than invented source history. Historical dimension classification is unavailable.

The published fixture has 1,000 customers, 120 products, 5,000 receipt headers,
14,995 eligible items from 15,000 raw items, 20 stores and 1,830 calendar dates.
QualityReport's existing JSON violations representation gains optional `details`:
the remediation finding holds the policy, `allow_degraded` gate, raw/eligible counts,
quarantine denominator, anonymous/header accounting and measured excluded item net
amounts by currency/date. The canonical gate decision is `passed` after revalidation;
it does not mean complete source relationships. No waiver is applied. Business impact
and source-history completeness remain unknown; basket/products remain degraded.
Receipt metrics must aggregate headers without item fan-out and cannot infer
product/category/brand revenue slicing from directory availability.

Semantic `impact_summary` pins this accounting, raw/quarantine artifact references,
canonical hashes, exact Calendar and Store projections, available dates/currencies,
and schema/mapping/relationship policy versions. These are provider projections;
consumers must first resolve current workspace authorization. No DSN or SQL is
exposed. Artifact dependencies retain raw-to-derived lineage; committed replay
revalidates all 13 artifact hashes. No schema migration is needed for this additive
versioned JSON profile. Existing historical full-retail publications remain unchanged.

[Preparation commands and session contract](../architecture/development-runtime-contract.md#11-prepared-retail-report-source-and-analyst-ms-003s01)
and [S01 evidence](../../.codex/delivery/evidence/MS-003/MS-003-S01/report.md)
record the observed boundary. Report persistence, the analyst UI and packaged HTTPS
remain S02–S05 responsibilities under the accepted milestone.
