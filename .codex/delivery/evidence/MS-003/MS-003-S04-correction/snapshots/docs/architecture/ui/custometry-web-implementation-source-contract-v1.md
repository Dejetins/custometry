...
uses the preserved pilot's scoped stylesheet and SVG symbols. Seven CSS class
names are prefixed to isolate unrelated legacy shell rules; layout constants,
typography, controls, rail, tabs and drawer rules retain their source values.
The inspector stays docked on wide screens and overlays the report at the narrow
endpoint, following the source. Header Trust opens the source-style dialog.
Required title/source inputs live in the inspector; period/store/comparison use
compact toolbar menus. Unsupported controls are visibly disabled.

The ECharts adapter verifies persisted S03 projections before applying the
owner-selected pilot presentation (axes, palette, line and legend layout).
The canonical series, units and reference hashes remain unchanged. The pilot's
customer fixture metrics and channel groups are not invented for receipt data.
Login and library have no corresponding complete pilot screens; they use its
existing surface and control rules.

The owner rejected the previous visual adaptation and authorized correction.
[Correction evidence](../../../.codex/delivery/evidence/MS-003/MS-003-S04-correction/report.md)
supersedes the former visual-fidelity claim. Original S04 evidence and receipt
remain historical records; they do not certify this corrected source. The
correction does not execute S05 or supply S06 owner acceptance.
