...
!!! info "Состояние Foundation"
    Текущий исполняемый контур проверяет Web-каркас, локальную документацию, health-границу API, миграцию control-plane PostgreSQL и детерминированный demo source. Ограниченный сценарий отчёта о продажах описан отдельно ниже; остальные продуктовые возможности не входят в этот базовый контур.
...
[Вход и работа с отчётом о продажах](user-guide/reports.md). Справка описывает ограниченный набор функций отчёта; наличие в установленной сборке зависит от её версии.
