#!/usr/bin/env python3
"""Prepare deterministic owner-acceptance evidence and ledger candidates for G4 RPT r5."""
...
spec = importlib.util.spec_from_file_location("accept_g4_rpt_base", SOURCE)
...
base.ARTIFACT = HERE / "artifacts/g4-r5/family-rpt-shell-workspace-baseline"
base.EVIDENCE = HERE / "evidence/family.rpt.shell-workspace.baseline-r5"
base.PREFLIGHT = base.EVIDENCE / "preflight"
base.REPORT = base.ROOT / ".codex/delivery/evidence/custometry-ui-design-program-v2/family.rpt.shell-workspace.baseline-r5-report.md"
base.STAGE = "G4@family.rpt.shell-workspace.baseline-r5"
base.NEXT_STAGE = "G4@family.pipe.shell-workspace.baseline-r5"
base.NEXT_PROMPT = base.ROOT / ".codex/agents/generated/custometry-ui-design-g0-v2/40-g4-12-family-pipe-shell-workspace-baseline-r5.md"
...
base.OWNER_INPUT = "ок принимается"
...
def owner_requests() -> None:
    base.dump(base.EVIDENCE / "family-acceptance-decision-r5-request.json", {
        ...
        "decision_text": (
            ...
            "Reports family. Acceptance covers UI-RPT-001 in all six required states, the four "
            "responsive-Web anchors, all five visible isolated actions, Result Trust, and "
            "applicable pilot visual-language inheritance. It does not authorize production "
            "implementation, publication, deployment, mobile-specific design, the adjacent "
            "PIPE family, G5, G6, or full WCAG conformance."
        ),
        "owner_input": base.OWNER_INPUT,
        "target": {
            "artifact_kind": "review_board",
            "artifact_id": "family.rpt.shell-workspace.baseline-r5",
            "revision": 5,
...
def acceptance_request() -> None:
    ...
    base.dump(base.EVIDENCE / "resolved-owner-decisions.json", [{
        ...
        "status": "resolved",
        "summary": "The owner explicitly accepted the complete Reports r5 HTML review board.",
...
def transition_inputs() -> None:
    ...
    if family.get("result") != "passed" or family.get("owner_decision_ref") != base.rel(base.DECISION):
        raise ValueError("strict accepted reports family aggregate is not current")
    ...
    base.dump(base.EVIDENCE / "stage-transition-request.json", {
        ...
        "from_gate": "G4",
        "from_target": "family.rpt.shell-workspace.baseline-r5",
        "from_revision": 5,
        "artifact": base.rel(base.FAMILY),
        "to_gate": "G4",
        "to_stage_id": base.NEXT_STAGE,
        "to_target": "family.pipe.shell-workspace.baseline-r5",
        ...
        "summary": "The owner accepted the complete Reports HTML review board; the strict family gate and adjacent-stage preflight pass.",
...
# G4 reports family accepted report

The owner accepted the complete hash-bound HTML review board for `UI-RPT-001`,
covering all six required states, all five isolated actions, and responsive-Web
...
execution of the adjacent PIPE family.
""", encoding="utf-8")


def transition_assembly_candidate() -> None:
    """Build a validation candidate while preserving the canonical triad link."""
    ...
    text = base.update_detail(text, base.NEXT_STAGE, {"execution_allowed": "true"})
