...
NEXT_PROMPT = ROOT / ".codex/agents/generated/custometry-ui-design-g0-v2/40-g4-08-family-fcst-shell-workspace-baseline-r5.md"
...
OWNER_INPUT = "Окей, это принимается. вся вот эта вот. Всё, что ты показал в HTML, это принимается."
...
def owner_requests() -> None:
    dump(EVIDENCE / "family-acceptance-decision-r5-request.json", {
        ...
        "status": "accepted",
        "revision": 5,
        "decision_text": "The owner explicitly accepted the complete HTML review board shown for the G4 Segmentation family. Acceptance covers UI-SEG-001 in all six required states, the three responsive-Web anchors, the visible isolated actions, Result Trust, and applicable pilot visual-language inheritance. It does not authorize production implementation, publication, deployment, mobile-specific design, another G4 family, G5, G6, or full WCAG conformance.",
        "owner_input": OWNER_INPUT,
        "target": {
            "artifact_kind": "review_board",
            "artifact_id": "family.seg.shell-workspace.baseline-r5",
            "revision": 5,
        ...
        "accepted_values": [{
            ...
            "screen_ids": ["UI-SEG-001"],
            "required_states": list(STATES),
            "responsive_web_anchors": [768, 1440, 1920],
            "source_bound_fixture_action_count": 3,
...
def review_evidence_candidate() -> None:
    text = LEDGER.read_text(encoding="utf-8")
    text = update_row(text, STAGE, {6: rel(BOARD)})
    (EVIDENCE / "review-evidence-candidate.md").write_text(text, encoding="utf-8")
...
def acceptance_request() -> None:
    request = json.loads((EVIDENCE / "family-acceptance-request.json").read_text(encoding="utf-8"))
    ...
    dump(EVIDENCE / "resolved-owner-decisions.json", [{
        ...
        "status": "resolved",
        "summary": "The owner explicitly accepted the complete Segmentation r5 HTML review board.",
...
def transition_inputs() -> None:
    ...
    sys.path.insert(0, str(Path("/Users/daniildegtyarev/.codex/skills/ui-design-program/scripts")))
    ...
    dump(EVIDENCE / "stage-transition-request.json", {
        ...
        "summary": "The owner accepted the complete Segmentation HTML review board; the strict family gate and adjacent-stage preflight pass.",
...
# G4 segmentation family accepted report

The owner accepted the complete hash-bound HTML review board for `UI-SEG-001`,
covering all six required states and responsive-Web anchors 768, 1440, and 1920.
...
def main() -> None:
    ...
    parser.add_argument("phase", choices=("owner-requests", "review-evidence-candidate", "resume-candidate", "acceptance-request", "transition-inputs", "accepted-candidate"))
    phase = parser.parse_args().phase
    {"owner-requests": owner_requests, "review-evidence-candidate": review_evidence_candidate, "resume-candidate": resume_candidate, "acceptance-request": acceptance_request, "transition-inputs": transition_inputs, "accepted-candidate": accepted_candidate}[phase]()
