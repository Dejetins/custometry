#!/usr/bin/env python3
"""Build blueprint-rebound G2 r4 structure from the G1 r4 atlas."""
...
LEGACY_WRITE_TEXT = legacy.write_text
G2 = PROGRAM_DIR / "artifacts/g2-r4"
EVIDENCE = PROGRAM_DIR / "evidence/CUSTOMETRY-UI-DESIGN-PROGRAM-V2-r4"
...
G1_BINDINGS_REL = ".codex/delivery/ui-design-programs/custometry-v2/artifacts/g1-r4/promoted-requirement-bindings.json"
G3_PROOF_SEED_REL = ".codex/delivery/ui-design-programs/custometry-v2/artifacts/g2-r4/g3-rendered-proof-input-seed.json"
...
def build_foundation_inheritance(
    families: list[dict[str, Any]], screens_by_id: dict[str, dict[str, Any]],
...
def build_families(screens, intake_by_id):
    ...
    if not isinstance(auth_family, dict):
        raise ValueError("one auth family must cover UI-AUTH-001 and UI-AUTH-004")
    auth_family["representative_screen_ids"] = ["UI-AUTH-001", "UI-AUTH-004"]
    auth_family["decision_origin"] = legacy.product_ref(f"{INTAKE_REL}#/screens")
...
def replace_paths(value: Any) -> Any:
    if isinstance(value, str):
        return value.replace("/artifacts/g2-r2/", "/artifacts/g2-r4/")
...
def write_text(path: Path, text: str) -> None:
    historical = ROOT / ".codex/delivery/evidence/custometry-ui-design-program-v2/g2-r2-structure-report.md"
    target = ROOT / ".codex/delivery/evidence/custometry-ui-design-program-v2/g2-r4-structure-report.md"
    LEGACY_WRITE_TEXT(target if path.resolve() == historical.resolve() else path, text)
...
def main() -> None:
    ...
    for path in [
        G2 / "screens-index.json", G2 / "journeys-index.json",
        G2 / "families-index.json", G2 / "waves-index.json",
        G2 / "coverage-profiles.json", G2 / "promoted-structure-bindings.json",
        ROOT / G3_PROOF_SEED_REL,
    ]:
        patch_document(path, lambda document: document.update({"program_revision": 5}))
    ...
    for source in program["source_contracts"]:
        if "/artifacts/g2-r4/journeys/" in source.get("path", ""):
    ...
    for screen in program["screens"]:
        if screen["screen_ref"]["expected_id"] in {"UI-AUTH-001", "UI-AUTH-004"}:
    ...
    summary.update({
        "program_revision": 5,
        "stage_instance_id": "G2@CUSTOMETRY-UI-DESIGN-PROGRAM-V2-r4",
        ...
        "semantic_rebind": {
            ...
            "filter_requirements": ["FILTER-011", "FILTER-012"],
            "comparison_requirements": ["COMPARE-008", "COMPARE-009", "COMPARE-010"],
            "status": "passed",
        },
