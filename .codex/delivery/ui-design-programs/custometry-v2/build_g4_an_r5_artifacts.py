#!/usr/bin/env python3
"""Build owned G4 analytics research artifacts for the r5 semantic successor."""
...
ARTIFACT_DIR = PROGRAM_DIR / "artifacts/g4-r5/family-an-shell-workspace-baseline"
REVIEW_ASSET_DIR = ARTIFACT_DIR / "review-assets"
EVIDENCE_DIR = PROGRAM_DIR / "evidence/family.an.shell-workspace.baseline-r5"
REPORT = ROOT / ".codex/delivery/evidence/custometry-ui-design-program-v2/family.an.shell-workspace.baseline-r5-report.md"
...
NEXT_PROMPT = ROOT / ".codex/agents/generated/custometry-ui-design-g0-v2/40-g4-07-family-seg-shell-workspace-baseline-r5.md"
...
STAGE_ID = "G4@family.an.shell-workspace.baseline-r5"
FAMILY_ID = "family.an.shell-workspace.baseline"
FAMILY_REVISION_ID = "family.an.shell-workspace.baseline-r5"
...
ARTIFACT_REF = ".codex/delivery/ui-design-programs/custometry-v2/artifacts/g4-r5/family-an-shell-workspace-baseline"
EVIDENCE_REF = ".codex/delivery/ui-design-programs/custometry-v2/evidence/family.an.shell-workspace.baseline-r5"

STATE_ORDER = ("initial", "loading", "populated", "error", "permission_denied", "recovery")
STATE_COPY = {
    "initial": {
        "eyebrow": "Аналитика · исследования",
        "title": "Исследования готовы к первому обзору",
        "summary": "Безопасный старт списка исследований без production-запросов и внешних изменений.",
        "tone": "neutral",
        "status": "Подготовка research workspace",
    },
    "loading": {
        "eyebrow": "Аналитика · исследования",
        "title": "Загружаем исследования",
        "summary": "Shell, Result Trust и контекст совместной проверки сохранены, пока fixture собирает список.",
        "tone": "neutral",
        "status": "Загрузка · без внешних запросов",
    },
    "populated": {
        "eyebrow": "Аналитика · исследования",
        "title": "Исследования и находки собраны",
        "summary": "Список связывает исследование, находки, комментарии и Result Trust в одной проверяемой поверхности.",
        "tone": "success",
        "status": "12 исследований · 4 на проверке · 7 находок",
    },
    "error": {
        "eyebrow": "Аналитика · исследования",
        "title": "Список исследований временно недоступен",
        "summary": "Контекст проверки и последний trust snapshot сохранены; fixture-ошибка не меняет рабочие данные.",
        "tone": "danger",
        "status": "Ошибка чтения snapshot · безопасный повтор доступен",
    },
    "permission_denied": {
        "eyebrow": "Аналитика · исследования",
        "title": "Недостаточно прав для исследований",
        "summary": "Закрытые исследования, находки и комментарии не раскрываются; shell-навигация сохранена.",
        "tone": "warning",
        "status": "Требуется research.manage или разрешение на комментарии",
    },
    "recovery": {
        "eyebrow": "Аналитика · исследования",
        "title": "Восстанавливаем research workspace",
        "summary": "Последний доверенный список и явный путь к безопасному продолжению совместной проверки.",
        "tone": "warning",
        "status": "Research snapshot r-219 · частично восстановлен",
    },
}
...
def planned_standard_targets() -> list[dict[str, Any]]:
    return [
        ...
        {"target_id": "research-action", "selector": "#research-action", "component_id": "action.button", "variant": "button", "size_class": "standard", "slot_id": "shell.main-content.button", "state_id": "default", "shell_region_id": "UI-AN-013.content", "icon_id": None, "element_type": "button"},
        {"target_id": "finding-action", "selector": "#finding-action", "component_id": "action.button", "variant": "button", "size_class": "standard", "slot_id": "shell.main-content.button", "state_id": "default", "shell_region_id": "UI-AN-013.content", "icon_id": None, "element_type": "button"},
        {"target_id": "comments-action", "selector": "#comments-action", "component_id": "action.button", "variant": "button", "size_class": "standard", "slot_id": "shell.main-content.button", "state_id": "default", "shell_region_id": "UI-AN-013.content", "icon_id": None, "element_type": "button"},
        {"target_id": "add-comment-action", "selector": "#add-comment-action", "component_id": "action.button", "variant": "button", "size_class": "standard", "slot_id": "shell.main-content.button", "state_id": "default", "shell_region_id": "UI-AN-013.content", "icon_id": None, "element_type": "button"},
        {"target_id": "resolve-comment-action", "selector": "#resolve-comment-action", "component_id": "action.button", "variant": "button", "size_class": "standard", "slot_id": "shell.main-content.button", "state_id": "default", "shell_region_id": "UI-AN-013.content", "icon_id": None, "element_type": "button"},
        {"target_id": "continue-action", "selector": "#continue-action", "component_id": "action.button", "variant": "button", "size_class": "standard", "slot_id": "shell.main-content.button", "state_id": "default", "shell_region_id": "UI-AN-013.content", "icon_id": None, "element_type": "button"},
    ]
...
def state_body(state: str) -> str:
    ...
    if state == "loading":
        primary = """
          <section class="state-panel loading-panel" aria-label="Загрузка исследований">
            <span class="skeleton wide"></span><span class="skeleton"></span><span class="skeleton"></span>
            <p>Собираем исследования, находки, комментарии и trust-метаданные в изолированном fixture.</p>
          </section>"""
        ...
        primary = """
          <section class="state-panel permission-panel" role="status">
            ...
            <h2>Исследования скрыты политикой workspace</h2>
            <p>Список, находки и комментарии не раскрываются. Нужны разрешения research.manage и comment.*.</p>
          </section>"""
    elif state == "error":
        primary = """
          <section class="state-panel error-panel" role="alert">
            <span class="status-mark">Fixture error</span>
            <h2>Не удалось прочитать research snapshot</h2>
            <p>Последний доверенный контекст сохранён. Повтор остаётся fixture-only и не меняет рабочие исследования.</p>
          </section>"""
    elif state == "recovery":
        primary = """
          <section class="state-panel recovery-panel" role="status">
            <span class="status-mark">Recovery mode</span>
            <h2>Используется последний доверенный research snapshot</h2>
            <p>Список отстаёт на 18 минут; изменения и комментарии доступны только после безопасной повторной проверки.</p>
          </section>"""
    elif state == "initial":
        primary = """
          <section class="state-panel initial-panel" role="status">
            <span class="status-mark">Первый вход</span>
            <h2>Research workspace готов к первому безопасному чтению</h2>
            <p>Структура, Result Trust и recovery controls уже на месте; production datasets не запрашиваются.</p>
          </section>"""
    else:
        primary = """
          <section class="kpi-module" aria-labelledby="research-kpi-title">
            <div class="kpi-module-header"><div class="kpi-module-title"><strong id="research-kpi-title">Research</strong><span class="kpi-context">trusted review</span></div></div>
            <div class="kpi-strip" aria-label="Сводка исследований">
              <article class="kpi"><span class="kpi-label">Исследования</span><div class="kpi-value-row"><strong class="kpi-value">12</strong><span class="kpi-delta">в fixture</span></div><span class="kpi-meta">активный список</span></article>
              <article class="kpi"><span class="kpi-label">На проверке</span><div class="kpi-value-row"><strong class="kpi-value">4</strong><span class="kpi-delta">33%</span></div><span class="kpi-meta">ожидают review</span></article>
              <article class="kpi"><span class="kpi-label">Находки</span><div class="kpi-value-row"><strong class="kpi-value">7</strong><span class="kpi-delta tone-warning">3 открыты</span></div><span class="kpi-meta">доказательства и выводы</span></article>
              <article class="kpi"><span class="kpi-label">Комментарии</span><div class="kpi-value-row"><strong class="kpi-value">18</strong><span class="kpi-delta tone-muted">5 новых</span></div><span class="kpi-meta">совместная проверка</span></article>
            </div>
          </section>
          <section class="overview-grid">
            <article class="data-card trust-card"><header><div><span class="section-kicker">Result Trust</span><h2>Доверие к исследованию</h2></div><span class="trust-score">Высокое</span></header><dl><div><dt>Snapshot</dt><dd>research-r-219</dd></div><div><dt>Freshness</dt><dd>8 минут</dd></div><div><dt>Evidence</dt><dd>7 находок</dd></div></dl></article>
            <article class="data-card checklist"><header><div><span class="section-kicker">Collaboration</span><h2>Контроль проверки</h2></div><span>6 / 6</span></header><ul><li class="done">research.manage и finding.manage проверены</li><li class="done">comment.read/create/resolve и продолжение пути сохранены</li></ul></article>
          </section>
          <section class="data-card run-card"><header><div><span class="section-kicker">Research list</span><h2>Совместная проверка</h2></div><span class="quiet-pill">fixture</span></header><div class="table-wrap"><table><thead><tr><th>Исследование</th><th>Статус</th><th>Находки</th><th>Комментарии</th></tr></thead><tbody><tr><td>Спрос по регионам</td><td>На проверке</td><td><span class="result success">3 подтверждены</span></td><td>8</td></tr><tr><td>Промо-эластичность</td><td>Требует ответа</td><td><span class="result warning">2 открыты</span></td><td>6</td></tr><tr><td>Сезонность категории</td><td>Готово</td><td><span class="result success">2 подтверждены</span></td><td>4</td></tr></tbody></table></div></section>"""
    return primary
...
def render_html(state: str, fixture_sha: str, font_sha: str, asset_sha: str, standard_rules: str) -> str:
    ...
    actions = (
        ("research-action", "UI-AN-013.research.manage", "Управлять исследованиями", "Управление исследованиями открыто в fixture"),
        ("finding-action", "UI-AN-013.finding.manage", "Управлять находками", "Управление находками открыто в fixture"),
        ("comments-action", "UI-AN-013.comment.read", "Открыть комментарии", "Комментарии открыты в fixture"),
        ("add-comment-action", "UI-AN-013.comment.create", "Добавить комментарий", "Комментарий добавлен в fixture"),
        ("resolve-comment-action", "UI-AN-013.comment.resolve", "Закрыть комментарий", "Комментарий закрыт в fixture"),
        ("continue-action", "journey.publish-review-collaborate.1", "Продолжить проверку", "Переход к следующему review готов"),
    )
    action_markup = "".join(
        f'<button id="{element_id}" type="button" data-ui-element="{element_id}" data-ui-component="action.button" data-ui-variant="secondary" data-ui-size="sm" data-ui-state="default" data-ui-slot="shell.main-content.button" data-prototype-action="{action_id}">{html.escape(label)}</button>'
        for element_id, action_id, label, _ in actions
    )
    action_script = "".join(
        f"document.getElementById('{element_id}').addEventListener('click',()=>{{result.textContent={json.dumps(outcome, ensure_ascii=False)};}});"
        for element_id, _, _, outcome in actions
    )
...
{favicon}  <meta name="ui-fixture-sha256" content="{fixture_sha}">
  ...
  <title>Custometry · Research · {safe_state}</title>
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .hero-actions button {{ justify-content: center; align-items: center; cursor: pointer; white-space: nowrap; }}
  ...
  <div id="app-shell" data-ui-region="shell.application" data-ui-element="shell-application">
    ...
    <section class="workspace">
      <header id="page-header" data-ui-region="shell.page-header" data-ui-element="page-header">
        <div class="header-copy"><span class="eyebrow">Custometry · North Star</span><h1>Исследования</h1></div>
      ...
      <main id="overview-main" data-ui-region="shell.main-content" data-ui-element="overview-main">
        <article id="overview-content" data-ui-region="UI-AN-013.content" data-ui-element="overview-content" data-state="{safe_state}">
          <section class="hero">
            <div><span class="eyebrow">{html.escape(copy['eyebrow'])}</span><h2>{html.escape(copy['title'])}</h2><p>{html.escape(copy['summary'])}</p></div>
            <div><div class="hero-actions">{action_markup}</div><p id="inspect-result" role="status" aria-live="polite"></p></div>
          </section>
{state_body(state)}
  ...
  <script>const result=document.getElementById('inspect-result');{action_script}</script>
</body>
</html>
"""
...
def element(
    ...
    *,
    ...
    return {
        ...
        "source_refs": [origin("product_contract", f"{INTAKE_REF}#/screens/55")],
    }
...
def screen_contract(state: str, html_path: Path, fixture_path: Path, baseline: dict[str, Any], intake_sha: str, baseline_sha: str, font_sha: str, asset_sha: str) -> dict[str, Any]:
    state_id = f"UI-AN-013.{state}"
    revision_id = f"UI-AN-013.{state}.all.ru.graphite@r5"
    ...
    elements = [
        ...
        element(state_id, element_id="overview-content", parent="overview-main", region_id="UI-AN-013.content", locator="#overview-content", element_type="article"),
    ...
    action_specs = (
        ("research-action", "UI-AN-013.research.manage", "Research Manage completes or the next contract-bound surface opens", "Управление исследованиями открыто в fixture"),
        ("finding-action", "UI-AN-013.finding.manage", "Finding Manage completes or the next contract-bound surface opens", "Управление находками открыто в fixture"),
        ("comments-action", "UI-AN-013.comment.read", "Comment Read completes or the next contract-bound surface opens", "Комментарии открыты в fixture"),
        ("add-comment-action", "UI-AN-013.comment.create", "Comment Create completes or the next contract-bound surface opens", "Комментарий добавлен в fixture"),
        ("resolve-comment-action", "UI-AN-013.comment.resolve", "Comment Resolve completes or the next contract-bound surface opens", "Комментарий закрыт в fixture"),
        ("continue-action", "journey.publish-review-collaborate.1", "Continue publish review collaborate completes or the next contract-bound surface opens", "Переход к следующему review готов"),
    )
    elements.extend([
        element(
            state_id,
            ...
            standard_slot_id="shell.main-content.button",
            standard_identity=standard_identity("action.button", "button", "standard", "button"),
        )
    ...
    visible_actions = [
        {
            ...
            "outcome_assertion": {"kind": "dom", "selector": "#inspect-result", "property": "textContent", "expected": fixture_outcome},
        }
        for _, action_id, expected_outcome, fixture_outcome in action_specs
    ]
    return {
        ...
        "functional_contract_ref": {"path": INTAKE_REF, "sha256": intake_sha, "json_pointer": "/screens/55", "screen_id": "UI-AN-013"},
        ...
        "standard_exceptions": [],
        "status": "review",
        "product_identity": {"screen_id": "UI-AN-013", "route_id": "UI-AN-013", "route": "/w/:workspaceKey/research", "state_id": state_id, "role_id": "AN", "permission_profile": "research.manage", "locale": "ru", "theme": "graphite"},
        ...
        "regions": [
            ...
            region("UI-AN-013.content", "#overview-content", "one_off", f"{INTAKE_REF}#/screens/55/regions/0"),
        ],
...
def build_program_snapshot() -> Path:
    ...
    matched = [row for row in projection["screens"] if row.get("screen_id") == "UI-AN-013"]
    if len(matched) != 1:
        raise ValueError("G1 projection cannot resolve exactly one UI-AN-013 screen")
    matched[0]["required_states"] = [f"UI-AN-013.{state}" for state in STATE_ORDER]
    ...
    snapshot["source_contracts"].append({
        ...
        "authority": "G4 stage-owned exact full-inventory projection adding only accepted required states for UI-AN-013",
    ...
    write_json(ARTIFACT_DIR / "program-snapshot-binding.json", {
        ...
        "reason": "Family aggregation adds exact accepted required_states only for the current UI-AN-013 representative; no other inventory meaning changes.",
...
Готова family review board для `UI-AN-013`: все шесть обязательных состояний,
...
def transition_inputs() -> None:
    ...
    checks = {
        ...
        "next_stage_inputs": {"next_stage_id": "G4@family.seg.shell-workspace.baseline-r5", "task_ref": next_prompt_ref, "unresolved_inputs": []},
    ...
    request = {
        ...
        "to_gate": "G4",
        "to_stage_id": "G4@family.seg.shell-workspace.baseline-r5",
        "to_target": "family.seg.shell-workspace.baseline-r5",
        ...
        "summary": "Research exact-covers all six required states and four responsive-Web anchors; machine proof is ready for owner review.",
...
gate_id: G4
status: review_ready
proof_boundary: current-contract isolated browser evidence for six UI-AN-013 states and four responsive-Web anchors; no production implementation, publication, deployment, mobile-specific design, full WCAG conformance, persistence, authorization, or performance proof
---
...
- `UI-AN-013` only; required states: `{', '.join(STATE_ORDER)}`.
...
- Isolated fixture interactions for all six source-bound `UI-AN-013` actions in the four applicable states; no external writes.
...
def acceptance_request() -> None:
    ...
    write_json(EVIDENCE_DIR / "resolved-owner-decisions.json", [{
        ...
        "status": "resolved",
        "summary": "The owner explicitly accepted the exact corrected Research r5 review board.",
...
def acceptance_transition_inputs() -> None:
    ...
    request = {
        ...
        "checks": [f"{check_id}={rel(preflight_dir / f'{check_id}.json')}" for check_id in ("current_gate", "source_freshness", "next_stage_inputs", "write_scope", "foreign_changes", "execution_route", "handoff_artifact")],
        "summary": "The owner accepted the exact finished Research family; the strict family gate and adjacent-stage preflight pass.",
        "review_artifacts": [rel(board_path), rel(EVIDENCE_DIR / "review-board-1440.png"), rel(EVIDENCE_DIR / "browser-board-qa.json")],
        "questions": [],
    }
