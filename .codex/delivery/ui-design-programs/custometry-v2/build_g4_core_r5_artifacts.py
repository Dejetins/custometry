#!/usr/bin/env python3
"""Build owned G4 core-overview artifacts for the r5 semantic successor."""
...
REPORT = ROOT / ".codex/delivery/evidence/custometry-ui-design-program-v2/family.core.shell-workspace.baseline-r5-report.md"
...
NEXT_PROMPT = ROOT / ".codex/agents/generated/custometry-ui-design-g0-v2/40-g4-04-family-data-shell-workspace-baseline-r5.md"
...
STAGE_ID = "G4@family.core.shell-workspace.baseline-r5"
FAMILY_ID = "family.core.shell-workspace.baseline"
FAMILY_REVISION_ID = "family.core.shell-workspace.baseline-r5"
...
ARTIFACT_REF = ".codex/delivery/ui-design-programs/custometry-v2/artifacts/g4-r5/family-core-shell-workspace-baseline"
EVIDENCE_REF = ".codex/delivery/ui-design-programs/custometry-v2/evidence/family.core.shell-workspace.baseline-r5"

STATE_ORDER = ("initial", "loading", "populated", "error", "permission_denied", "recovery")
STATE_COPY = {
    "initial": {
        "eyebrow": "Рабочее пространство · обзор",
        "title": "Добро пожаловать в Custometry",
        "summary": "Собираем безопасный обзор источников, качества и последних запусков.",
        "tone": "neutral",
        "status": "Подготовка overview",
    },
    "loading": {
        "eyebrow": "Рабочее пространство · обзор",
        "title": "Обновляем показатели",
        "summary": "Сохраняем навигацию и контекст, пока fixture-данные проходят проверку.",
        "tone": "neutral",
    ...
    "populated": {
        ...
        "summary": "Один спокойный обзор свежести, качества, запусков и прогнозных сигналов.",
        "tone": "success",
        "status": "Данные актуальны · 14 фев 2026, 10:00",
    },
    "error": {
        "eyebrow": "Рабочее пространство · обзор",
        "title": "Overview временно недоступен",
        "summary": "Контекст сохранён. Ошибка fixture не создаёт ложный terminal state.",
        "tone": "danger",
        "status": "Ошибка чтения snapshot · безопасный повтор доступен",
    },
    "permission_denied": {
        "eyebrow": "Рабочее пространство · обзор",
        "title": "Недостаточно прав для данных overview",
        "summary": "Скрытые значения и counts не раскрываются; навигация остаётся доступной.",
        "tone": "warning",
    ...
    "recovery": {
        ...
        "summary": "Показываем последний доверенный snapshot и явный путь безопасного повтора.",
        "tone": "warning",
...
def planned_standard_targets() -> list[dict[str, Any]]:
    return [
        ...
        {"target_id": "inspect-action", "selector": "#inspect-action", "component_id": "action.button", "variant": "button", "size_class": "standard", "slot_id": "shell.main-content.button", "state_id": "default", "shell_region_id": "UI-CORE-001.content", "icon_id": None, "element_type": "button"},
    ]
...
def state_body(state: str) -> str:
    ...
    if state == "loading":
        primary = """
          <section class="state-panel loading-panel" aria-label="Загрузка показателей">
            <span class="skeleton wide"></span><span class="skeleton"></span><span class="skeleton"></span>
            <p>Проверяем freshness, quality и последние запуски в изолированном fixture.</p>
          </section>"""
        ...
        primary = """
          <section class="state-panel permission-panel" role="status">
            ...
            <h2>Показатели скрыты политикой workspace</h2>
            <p>Ни агрегаты, ни скрытые counts не отображаются. Обратитесь к администратору рабочего пространства.</p>
          </section>"""
    elif state == "error":
        primary = """
          <section class="state-panel error-panel" role="alert">
            <span class="status-mark">Fixture error</span>
            <h2>Не удалось прочитать текущий snapshot</h2>
            <p>Последний доверенный контекст сохранён. Повтор не выполняет внешних записей.</p>
          </section>"""
    elif state == "recovery":
        primary = """
          <section class="state-panel recovery-panel" role="status">
            <span class="status-mark">Recovery mode</span>
            <h2>Используется последний доверенный snapshot v12</h2>
            <p>Freshness отстаёт на 18 минут; качество и lineage доступны для проверки.</p>
          </section>"""
    elif state == "initial":
        primary = """
          <section class="state-panel initial-panel" role="status">
            <span class="status-mark">Первый вход</span>
            <h2>Overview готовится к первому безопасному чтению</h2>
            <p>Структура, trust и recovery controls уже на месте; реальные данные не запрашиваются.</p>
          </section>"""
    else:
        primary = """
          <section class="kpi-module" aria-labelledby="workspace-kpi-title">
            <div class="kpi-module-header"><div class="kpi-module-title"><strong id="workspace-kpi-title">KPI</strong><span class="kpi-context">snapshot v12</span></div></div>
            <div class="kpi-strip" aria-label="Ключевые показатели workspace">
              <article class="kpi"><span class="kpi-label">Свежесть данных</span><div class="kpi-value-row"><strong class="kpi-value">8 мин</strong><span class="kpi-delta">в норме</span></div><span class="kpi-meta">snapshot v12</span></article>
              <article class="kpi"><span class="kpi-label">Качество</span><div class="kpi-value-row"><strong class="kpi-value">97%</strong><span class="kpi-delta tone-warning">2 проверки</span></div><span class="kpi-meta">2 проверки требуют внимания</span></article>
              <article class="kpi"><span class="kpi-label">Последние запуски</span><div class="kpi-value-row"><strong class="kpi-value">11 / 12</strong><span class="kpi-delta tone-muted">1 retry</span></div><span class="kpi-meta">один безопасный retry</span></article>
              <article class="kpi"><span class="kpi-label">Forecast alerts</span><div class="kpi-value-row"><strong class="kpi-value">2</strong><span class="kpi-delta tone-muted">без auto</span></div><span class="kpi-meta">без auto-promotion</span></article>
            </div>
          </section>
          <section class="overview-grid">
            <article class="data-card trust-card"><header><div><span class="section-kicker">Result Trust</span><h2>Доверие к результату</h2></div><span class="trust-score">Высокое</span></header><dl><div><dt>Snapshot</dt><dd>v12 · immutable</dd></div><div><dt>Metric group</dt><dd>workspace-overview-v5</dd></div><div><dt>Coverage</dt><dd>14 / 14 источников</dd></div></dl></article>
            <article class="data-card checklist"><header><div><span class="section-kicker">Onboarding</span><h2>Настройка workspace</h2></div><span>3 / 5</span></header><ul><li class="done">Workspace создан</li><li class="done">Источник подключён</li><li class="done">Quality gate настроен</li><li>Опубликовать metric</li><li>Пригласить команду</li></ul></article>
          </section>
          <section class="data-card run-card"><header><div><span class="section-kicker">Recent runs</span><h2>Последние запуски</h2></div><span class="quiet-pill">fixture</span></header><div class="table-wrap"><table><thead><tr><th>Запуск</th><th>Контур</th><th>Свежесть</th><th>Результат</th></tr></thead><tbody><tr><td>run-1208</td><td>Customer 360</td><td>10:00</td><td><span class="result success">Успешно</span></td></tr><tr><td>run-1207</td><td>Forecast weekly</td><td>09:42</td><td><span class="result warning">Внимание</span></td></tr><tr><td>run-1206</td><td>Quality daily</td><td>09:10</td><td><span class="result success">Успешно</span></td></tr></tbody></table></div></section>"""
    return primary
...
{favicon}  <meta name="ui-fixture-sha256" content="{fixture_sha}">
  ...
  <title>Custometry · Workspace Overview · {safe_state}</title>
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    #inspect-action {{ justify-content: center; align-items: center; cursor: pointer; white-space: nowrap; }}
  ...
  <div id="app-shell" data-ui-region="shell.application" data-ui-element="shell-application">
    ...
    <section class="workspace">
      <header id="page-header" data-ui-region="shell.page-header" data-ui-element="page-header">
        <div class="header-copy"><span class="eyebrow">Custometry · North Star</span><h1>Workspace Overview</h1></div>
      ...
      <main id="overview-main" data-ui-region="shell.main-content" data-ui-element="overview-main">
        <article id="overview-content" data-ui-region="UI-CORE-001.content" data-ui-element="overview-content" data-state="{safe_state}">
          <section class="hero">
            <div><span class="eyebrow">{html.escape(copy['eyebrow'])}</span><h2>{html.escape(copy['title'])}</h2><p>{html.escape(copy['summary'])}</p></div>
            <div><button id="inspect-action" type="button" data-ui-element="inspect-action" data-ui-component="action.button" data-ui-variant="secondary" data-ui-size="sm" data-ui-state="default" data-ui-slot="shell.main-content.button" data-prototype-action="UI-CORE-001.inspect">Проверить поверхность</button><p id="inspect-result" role="status" aria-live="polite"></p></div>
          </section>
{state_body(state)}
  ...
  <script>document.getElementById('inspect-action').addEventListener('click',()=>{{document.getElementById('inspect-result').textContent='Поверхность overview проверена';}});</script>
</body>
</html>
"""
...
def element(
    ...
    *,
    ...
    return {
        ...
        "source_refs": [origin("product_contract", f"{INTAKE_REF}#/screens/6")],
    }
...
def screen_contract(state: str, html_path: Path, fixture_path: Path, baseline: dict[str, Any], intake_sha: str, baseline_sha: str, font_sha: str, asset_sha: str) -> dict[str, Any]:
    state_id = f"UI-CORE-001.{state}"
    revision_id = f"UI-CORE-001.{state}.all.ru.graphite@r5"
    ...
    elements = [
        ...
        element(state_id, element_id="overview-content", parent="overview-main", region_id="UI-CORE-001.content", locator="#overview-content", element_type="article"),
        ...
        element(state_id, element_id="inspect-action", parent="overview-content", region_id="UI-CORE-001.content", locator="#inspect-action", element_type="button", component_id="action.button", variant="secondary", size_class="sm", action_ids=["UI-CORE-001.inspect"], standard_slot_id="shell.main-content.button", standard_identity=standard_identity("action.button", "button", "standard", "button")),
    ]
    return {
        ...
        "functional_contract_ref": {"path": INTAKE_REF, "sha256": intake_sha, "json_pointer": "/screens/6", "screen_id": "UI-CORE-001"},
        ...
        "standard_exceptions": [],
        "status": "review",
        "product_identity": {"screen_id": "UI-CORE-001", "route_id": "UI-CORE-001", "route": "/w/:workspaceKey/overview", "state_id": state_id, "role_id": "all", "permission_profile": "workspace.manage", "locale": "ru", "theme": "graphite"},
        ...
        "interaction_contract": {
            "visible_actions": [{"action_id": "UI-CORE-001.inspect", "expected_outcome": "Inspect current surface completes or the next contract-bound surface opens", "side_effect_class": "none", "activation_policy": "execute_in_fixture", "outcome_assertion": {"kind": "dom", "selector": "#inspect-result", "property": "textContent", "expected": "Поверхность overview проверена"}}],
        ...
        "regions": [
            ...
            region("UI-CORE-001.content", "#overview-content", "one_off", f"{INTAKE_REF}#/screens/6/regions/0"),
        ],
...
def build_program_snapshot() -> Path:
    ...
    matched = [row for row in projection["screens"] if row.get("screen_id") == "UI-CORE-001"]
    if len(matched) != 1:
        raise ValueError("G1 projection cannot resolve exactly one UI-CORE-001 screen")
    matched[0]["required_states"] = [f"UI-CORE-001.{state}" for state in STATE_ORDER]
    ...
    snapshot["source_contracts"].append({
        ...
        "authority": "G4 stage-owned exact full-inventory projection adding only accepted required states for UI-CORE-001",
    ...
    write_json(ARTIFACT_DIR / "program-snapshot-binding.json", {
        ...
        "reason": "Family aggregation adds exact accepted required_states only for the current UI-CORE-001 representative; no other inventory meaning changes.",
...
# Готовый Workspace Overview

## Что уже готово

Готова family review board для `UI-CORE-001`: все шесть обязательных состояний,
...
def transition_inputs() -> None:
    ...
    checks = {
        ...
        "next_stage_inputs": {"next_stage_id": "G4@family.data.shell-workspace.baseline-r5", "task_ref": next_prompt_ref, "unresolved_inputs": []},
    ...
    request = {
        ...
        "to_gate": "G4",
        "to_stage_id": "G4@family.data.shell-workspace.baseline-r5",
        "to_target": "family.data.shell-workspace.baseline-r5",
        ...
        "summary": "Workspace Overview exact-covers all six required states and four responsive-Web anchors; machine proof is ready for owner review.",
...
gate_id: G4
status: review_ready
proof_boundary: current-contract isolated browser evidence for six UI-CORE-001 states and four responsive-Web anchors; no production implementation, publication, deployment, mobile-specific design, full WCAG conformance, persistence, authorization, or performance proof
---

# G4 core overview family r5 execution report

## Scope

- `UI-CORE-001` only; required states: `{', '.join(STATE_ORDER)}`.
...
- Isolated fixture interaction for `UI-CORE-001.inspect`; no external writes.
...
def acceptance_request() -> None:
    ...
    write_json(EVIDENCE_DIR / "resolved-owner-decisions.json", [{
        ...
        "status": "resolved",
        "summary": "The owner explicitly accepted the exact corrected Workspace Overview r5 review board.",
...
def acceptance_transition_inputs() -> None:
    ...
    request = {
        ...
        "checks": [f"{check_id}={rel(preflight_dir / f'{check_id}.json')}" for check_id in ("current_gate", "source_freshness", "next_stage_inputs", "write_scope", "foreign_changes", "execution_route", "handoff_artifact")],
        "summary": "The owner accepted the exact corrected Workspace Overview family; the strict family gate and adjacent-stage preflight pass.",
        "review_artifacts": [rel(board_path), rel(EVIDENCE_DIR / "review-board-1440.png"), rel(EVIDENCE_DIR / "browser-board-qa.json")],
        "questions": [],
    }
...
gate_id: G4
status: ready
proof_boundary: accepted current-contract isolated browser evidence for six UI-CORE-001 states and four responsive-Web anchors; no production implementation, publication, deployment, mobile-specific design, full WCAG conformance, persistence, authorization, or performance proof
---
...
- `python3 .codex/delivery/ui-design-programs/custometry-v2/run_g4_core_r5_board_qa.py` — passed, including accepted-pilot KPI-row conformance and clean console/network observations.
- `uv run python -m tools.check --scope local` — passed.

## File manifest

- `created`:
  - `.codex/delivery/ui-design-programs/custometry-v2/artifacts/g4-r5/family-core-shell-workspace-baseline/**`
  - `.codex/delivery/ui-design-programs/custometry-v2/evidence/family.core.shell-workspace.baseline-r5/**`
  - `.codex/delivery/evidence/custometry-ui-design-program-v2/family.core.shell-workspace.baseline-r5-report.md`
  - `.codex/delivery/ui-design-programs/custometry-v2/build_g4_core_r5_artifacts.py`
  - `.codex/delivery/ui-design-programs/custometry-v2/run_g4_core_r5_board_qa.py`
  - `.codex/delivery/ui-design-programs/custometry-v2/run_g4_core_r5_proof.py`
- `modified`:
  - `.codex/delivery/ui-design-programs/custometry-v2/mutate_blueprint_rebind_r5_lifecycle.py`
  - `.codex/agents/generated/custometry-ui-design-g0-v2/40-g4-03-family-core-shell-workspace-baseline-r5.md`
