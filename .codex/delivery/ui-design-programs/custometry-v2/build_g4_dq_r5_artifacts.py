#!/usr/bin/env python3
"""Build owned G4 data-quality artifacts for the r5 semantic successor."""
...
ARTIFACT_DIR = PROGRAM_DIR / "artifacts/g4-r5/family-dq-shell-workspace-baseline"
REVIEW_ASSET_DIR = ARTIFACT_DIR / "review-assets"
EVIDENCE_DIR = PROGRAM_DIR / "evidence/family.dq.shell-workspace.baseline-r5"
REPORT = ROOT / ".codex/delivery/evidence/custometry-ui-design-program-v2/family.dq.shell-workspace.baseline-r5-report.md"
...
NEXT_PROMPT = ROOT / ".codex/agents/generated/custometry-ui-design-g0-v2/40-g4-06-family-an-shell-workspace-baseline-r5.md"
...
STAGE_ID = "G4@family.dq.shell-workspace.baseline-r5"
FAMILY_ID = "family.dq.shell-workspace.baseline"
FAMILY_REVISION_ID = "family.dq.shell-workspace.baseline-r5"
...
ARTIFACT_REF = ".codex/delivery/ui-design-programs/custometry-v2/artifacts/g4-r5/family-dq-shell-workspace-baseline"
EVIDENCE_REF = ".codex/delivery/ui-design-programs/custometry-v2/evidence/family.dq.shell-workspace.baseline-r5"
...
STATE_COPY = {
    "initial": {
        "eyebrow": "Данные · качество",
        "title": "Качество готово к первой проверке",
        "summary": "Безопасный старт без production-запросов и изменения правил.",
        "tone": "neutral",
        "status": "Подготовка quality overview",
    },
    "loading": {
        "eyebrow": "Данные · качество",
        "title": "Загружаем профиль качества",
        "summary": "Shell, Result Trust и recovery controls сохранены, пока fixture собирает сигналы.",
        "tone": "neutral",
        "status": "Загрузка · без внешних запросов",
    },
    "populated": {
        "eyebrow": "Данные · качество",
        "title": "Качество данных под контролем",
        "summary": "Сводка связывает freshness, coverage, failed checks и последний доверенный run.",
        "tone": "success",
        "status": "24 правила · 22 в норме · 2 требуют внимания",
    },
    "error": {
        "eyebrow": "Данные · качество",
        "title": "Результаты качества временно недоступны",
        "summary": "Контекст, правила и trust snapshot сохранены; fixture-ошибка не меняет production data.",
        "tone": "danger",
        "status": "Ошибка чтения snapshot · безопасный повтор доступен",
    },
    "permission_denied": {
        "eyebrow": "Данные · качество",
        "title": "Недостаточно прав для качества",
        "summary": "Правила, статусы и скрытые counts не раскрываются; shell-навигация сохранена.",
        "tone": "warning",
        "status": "Требуется quality.manage",
    },
    "recovery": {
        "eyebrow": "Данные · качество",
        "title": "Восстанавливаем quality overview",
        "summary": "Последний доверенный run и явный путь к безопасной повторной проверке.",
        "tone": "warning",
        "status": "Quality run q-184 · частично восстановлен",
    },
}
...
def planned_standard_targets() -> list[dict[str, Any]]:
    return [
        ...
        {"target_id": "manage-action", "selector": "#manage-action", "component_id": "action.button", "variant": "button", "size_class": "standard", "slot_id": "shell.main-content.button", "state_id": "default", "shell_region_id": "UI-DQ-001.content", "icon_id": None, "element_type": "button"},
        {"target_id": "continue-action", "selector": "#continue-action", "component_id": "action.button", "variant": "button", "size_class": "standard", "slot_id": "shell.main-content.button", "state_id": "default", "shell_region_id": "UI-DQ-001.content", "icon_id": None, "element_type": "button"},
    ]
...
def state_body(state: str) -> str:
    ...
    if state == "loading":
        primary = """
          <section class="state-panel loading-panel" aria-label="Загрузка качества данных">
            <span class="skeleton wide"></span><span class="skeleton"></span><span class="skeleton"></span>
            <p>Собираем freshness, coverage, failed checks и trust-метаданные в изолированном fixture.</p>
          </section>"""
        ...
        primary = """
          <section class="state-panel permission-panel" role="status">
            ...
            <h2>Профиль качества скрыт политикой workspace</h2>
            <p>Правила, статусы и counts не раскрываются. Нужна роль с правом quality.manage.</p>
          </section>"""
    elif state == "error":
        primary = """
          <section class="state-panel error-panel" role="alert">
            <span class="status-mark">Fixture error</span>
            <h2>Не удалось прочитать quality snapshot</h2>
            <p>Последний доверенный контекст сохранён. Повтор остаётся fixture-only и не меняет производственные правила.</p>
          </section>"""
    elif state == "recovery":
        primary = """
          <section class="state-panel recovery-panel" role="status">
            <span class="status-mark">Recovery mode</span>
            <h2>Используется последний доверенный quality run</h2>
            <p>Результаты отстают на 18 минут; управление правилами доступно только после безопасной повторной проверки.</p>
          </section>"""
    elif state == "initial":
        primary = """
          <section class="state-panel initial-panel" role="status">
            <span class="status-mark">Первый вход</span>
            <h2>Quality overview готов к первому безопасному чтению</h2>
            <p>Структура, Result Trust и recovery controls уже на месте; production datasets не запрашиваются.</p>
          </section>"""
    else:
        primary = """
          <section class="kpi-module" aria-labelledby="quality-kpi-title">
            <div class="kpi-module-header"><div class="kpi-module-title"><strong id="quality-kpi-title">Quality</strong><span class="kpi-context">trusted run</span></div></div>
            <div class="kpi-strip" aria-label="Сводка качества данных">
              <article class="kpi"><span class="kpi-label">Правила</span><div class="kpi-value-row"><strong class="kpi-value">24</strong><span class="kpi-delta">в fixture</span></div><span class="kpi-meta">полнота и freshness</span></article>
              <article class="kpi"><span class="kpi-label">В норме</span><div class="kpi-value-row"><strong class="kpi-value">22</strong><span class="kpi-delta">91,7%</span></div><span class="kpi-meta">прошли порог</span></article>
              <article class="kpi"><span class="kpi-label">Требуют внимания</span><div class="kpi-value-row"><strong class="kpi-value">2</strong><span class="kpi-delta tone-warning">проверить</span></div><span class="kpi-meta">orders и customers</span></article>
              <article class="kpi"><span class="kpi-label">Freshness</span><div class="kpi-value-row"><strong class="kpi-value">8м</strong><span class="kpi-delta tone-muted">q-184</span></div><span class="kpi-meta">последний run</span></article>
            </div>
          </section>
          <section class="overview-grid">
            <article class="data-card trust-card"><header><div><span class="section-kicker">Result Trust</span><h2>Доверие к результату</h2></div><span class="trust-score">Высокое</span></header><dl><div><dt>Run</dt><dd>quality-q-184</dd></div><div><dt>Freshness</dt><dd>8 минут</dd></div><div><dt>Coverage</dt><dd>24 правила</dd></div></dl></article>
            <article class="data-card checklist"><header><div><span class="section-kicker">Safe operation</span><h2>Контроль действий</h2></div><span>2 / 2</span></header><ul><li class="done">quality.manage проверен</li><li class="done">переход к Analytics сохраняет trust-контекст</li></ul></article>
          </section>
          <section class="data-card run-card"><header><div><span class="section-kicker">Quality checks</span><h2>Последний run</h2></div><span class="quiet-pill">fixture</span></header><div class="table-wrap"><table><thead><tr><th>Набор</th><th>Проверка</th><th>Порог</th><th>Результат</th></tr></thead><tbody><tr><td>orders</td><td>Полнота</td><td>≥ 99%</td><td><span class="result success">99,8%</span></td></tr><tr><td>customers</td><td>Свежесть</td><td>≤ 30 мин</td><td><span class="result warning">42 мин</span></td></tr><tr><td>products</td><td>Уникальность</td><td>100%</td><td><span class="result success">100%</span></td></tr></tbody></table></div></section>"""
    return primary
...
def render_html(state: str, fixture_sha: str, font_sha: str, asset_sha: str, standard_rules: str) -> str:
    ...
    action_markup = """<button id="manage-action" type="button" data-ui-element="manage-action" data-ui-component="action.button" data-ui-variant="secondary" data-ui-size="sm" data-ui-state="default" data-ui-slot="shell.main-content.button" data-prototype-action="UI-DQ-001.quality.manage">Управлять качеством</button><button id="continue-action" type="button" data-ui-element="continue-action" data-ui-component="action.button" data-ui-variant="secondary" data-ui-size="sm" data-ui-state="default" data-ui-slot="shell.main-content.button" data-prototype-action="journey.data-to-trusted-result.2">К доверенному результату</button>"""
    action_script = """document.getElementById('manage-action').addEventListener('click',()=>{result.textContent='Управление качеством открыто в fixture';});document.getElementById('continue-action').addEventListener('click',()=>{result.textContent='Переход к доверенному результату готов';});"""
...
{favicon}  <meta name="ui-fixture-sha256" content="{fixture_sha}">
  ...
  <title>Custometry · Data Quality · {safe_state}</title>
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    #manage-action, #continue-action {{ justify-content: center; align-items: center; cursor: pointer; white-space: nowrap; }}
  ...
  <div id="app-shell" data-ui-region="shell.application" data-ui-element="shell-application">
    ...
    <section class="workspace">
      <header id="page-header" data-ui-region="shell.page-header" data-ui-element="page-header">
        <div class="header-copy"><span class="eyebrow">Custometry · North Star</span><h1>Data Quality</h1></div>
      ...
      <main id="overview-main" data-ui-region="shell.main-content" data-ui-element="overview-main">
        <article id="overview-content" data-ui-region="UI-DQ-001.content" data-ui-element="overview-content" data-state="{safe_state}">
          <section class="hero">
...
def element(
    ...
    *,
    ...
    return {
        ...
        "source_refs": [origin("product_contract", f"{INTAKE_REF}#/screens/38")],
    }
...
def screen_contract(state: str, html_path: Path, fixture_path: Path, baseline: dict[str, Any], intake_sha: str, baseline_sha: str, font_sha: str, asset_sha: str) -> dict[str, Any]:
    state_id = f"UI-DQ-001.{state}"
    revision_id = f"UI-DQ-001.{state}.all.ru.graphite@r5"
    ...
    elements = [
        ...
        element(state_id, element_id="overview-content", parent="overview-main", region_id="UI-DQ-001.content", locator="#overview-content", element_type="article"),
    ...
    elements.extend([
        element(state_id, element_id="manage-action", parent="overview-content", region_id="UI-DQ-001.content", locator="#manage-action", element_type="button", component_id="action.button", variant="secondary", size_class="sm", action_ids=["UI-DQ-001.quality.manage"], standard_slot_id="shell.main-content.button", standard_identity=standard_identity("action.button", "button", "standard", "button")),
        element(state_id, element_id="continue-action", parent="overview-content", region_id="UI-DQ-001.content", locator="#continue-action", element_type="button", component_id="action.button", variant="secondary", size_class="sm", action_ids=["journey.data-to-trusted-result.2"], standard_slot_id="shell.main-content.button", standard_identity=standard_identity("action.button", "button", "standard", "button")),
    ])
    visible_actions = [
        {"action_id": "UI-DQ-001.quality.manage", "expected_outcome": "Quality Manage completes or the next contract-bound surface opens", "side_effect_class": "none", "activation_policy": "execute_in_fixture", "outcome_assertion": {"kind": "dom", "selector": "#inspect-result", "property": "textContent", "expected": "Управление качеством открыто в fixture"}},
        {"action_id": "journey.data-to-trusted-result.2", "expected_outcome": "Continue data to trusted result completes or the next contract-bound surface opens", "side_effect_class": "none", "activation_policy": "execute_in_fixture", "outcome_assertion": {"kind": "dom", "selector": "#inspect-result", "property": "textContent", "expected": "Переход к доверенному результату готов"}},
    ]
    return {
        ...
        "functional_contract_ref": {"path": INTAKE_REF, "sha256": intake_sha, "json_pointer": "/screens/38", "screen_id": "UI-DQ-001"},
        ...
        "standard_exceptions": [],
        "status": "review",
        "product_identity": {"screen_id": "UI-DQ-001", "route_id": "UI-DQ-001", "route": "/w/:workspaceKey/data-quality", "state_id": state_id, "role_id": "DS", "permission_profile": "quality.manage", "locale": "ru", "theme": "graphite"},
        ...
        "regions": [
            ...
            region("UI-DQ-001.content", "#overview-content", "one_off", f"{INTAKE_REF}#/screens/38/regions/0"),
        ],
...
def build_program_snapshot() -> Path:
    ...
    matched = [row for row in projection["screens"] if row.get("screen_id") == "UI-DQ-001"]
    if len(matched) != 1:
        raise ValueError("G1 projection cannot resolve exactly one UI-DQ-001 screen")
    matched[0]["required_states"] = [f"UI-DQ-001.{state}" for state in STATE_ORDER]
    ...
    snapshot["source_contracts"].append({
        ...
        "authority": "G4 stage-owned exact full-inventory projection adding only accepted required states for UI-DQ-001",
    ...
    write_json(ARTIFACT_DIR / "program-snapshot-binding.json", {
        ...
        "reason": "Family aggregation adds exact accepted required_states only for the current UI-DQ-001 representative; no other inventory meaning changes.",
...
# Готовый Data Quality

## Что уже готово

Готова family review board для `UI-DQ-001`: все шесть обязательных состояний,
...
def transition_inputs() -> None:
    ...
    checks = {
        ...
        "next_stage_inputs": {"next_stage_id": "G4@family.an.shell-workspace.baseline-r5", "task_ref": next_prompt_ref, "unresolved_inputs": []},
    ...
    request = {
        ...
        "to_gate": "G4",
        "to_stage_id": "G4@family.an.shell-workspace.baseline-r5",
        "to_target": "family.an.shell-workspace.baseline-r5",
        ...
        "summary": "Data Quality exact-covers all six required states and four responsive-Web anchors; machine proof is ready for owner review.",
...
gate_id: G4
status: review_ready
proof_boundary: current-contract isolated browser evidence for six UI-DQ-001 states and four responsive-Web anchors; no production implementation, publication, deployment, mobile-specific design, full WCAG conformance, persistence, authorization, or performance proof
---

# G4 data quality family r5 execution report

## Scope

- `UI-DQ-001` only; required states: `{', '.join(STATE_ORDER)}`.
...
- Isolated fixture interactions for both source-bound `UI-DQ-001` actions; no external writes.
...
def acceptance_request() -> None:
    ...
    write_json(EVIDENCE_DIR / "resolved-owner-decisions.json", [{
        ...
        "status": "resolved",
        "summary": "The owner explicitly accepted the exact corrected Data Quality r5 review board.",
...
def acceptance_transition_inputs() -> None:
    ...
    request = {
        ...
        "checks": [f"{check_id}={rel(preflight_dir / f'{check_id}.json')}" for check_id in ("current_gate", "source_freshness", "next_stage_inputs", "write_scope", "foreign_changes", "execution_route", "handoff_artifact")],
        "summary": "The owner accepted the exact corrected Data Quality family; the strict family gate and adjacent-stage preflight pass.",
        "review_artifacts": [rel(board_path), rel(EVIDENCE_DIR / "review-board-1440.png"), rel(EVIDENCE_DIR / "browser-board-qa.json")],
        "questions": [],
    }
...
gate_id: G4
status: ready
proof_boundary: accepted current-contract isolated browser evidence for six UI-DQ-001 states and four responsive-Web anchors; no production implementation, publication, deployment, mobile-specific design, full WCAG conformance, persistence, authorization, or performance proof
---
...
- `python3 .codex/delivery/ui-design-programs/custometry-v2/run_g4_dq_r5_board_qa.py` — passed, including accepted-pilot KPI-row conformance and clean console/network observations.
- `uv run python -m tools.check --scope local` — passed.

## File manifest

- `created`:
  - `.codex/delivery/ui-design-programs/custometry-v2/artifacts/g4-r5/family-dq-shell-workspace-baseline/**`
  - `.codex/delivery/ui-design-programs/custometry-v2/evidence/family.dq.shell-workspace.baseline-r5/**`
  - `.codex/delivery/evidence/custometry-ui-design-program-v2/family.dq.shell-workspace.baseline-r5-report.md`
  - `.codex/delivery/ui-design-programs/custometry-v2/build_g4_dq_r5_artifacts.py`
  - `.codex/delivery/ui-design-programs/custometry-v2/run_g4_dq_r5_board_qa.py`
  - `.codex/delivery/ui-design-programs/custometry-v2/run_g4_dq_r5_proof.py`
- `modified`:
  - `.codex/delivery/ui-design-programs/custometry-v2/mutate_blueprint_rebind_r5_lifecycle.py`
  - `.codex/agents/generated/custometry-ui-design-g0-v2/40-g4-05-family-dq-shell-workspace-baseline-r5.md`
