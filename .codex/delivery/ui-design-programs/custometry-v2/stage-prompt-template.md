...
ui_standard:
  standard_revision_id: <exact-pilot-derived-standard-revision>
  clause_inventory_sha256: <exact-clause-inventory-sha256>
  source_inventory_ref: <hash-pinned-source-inventory-ref>
  applicability_policy: independently_derived_exact_cover
  allowed_dispositions: [conforms, accepted_exception, source_backed_not_applicable]
...
The accepted pilot is normative for every applicable reusable UI detail. Its
hash-bound source inventory and stable clauses fix shared foundations, complete
typography, controls, component anatomy/states, spacing, radii, borders,
elevation, icons, focus/motion, shell/layout, and responsive adaptation. Novel
target semantics, content, data, actions, and screen-specific composition remain
target-owned. `visual_language_conformance` is clause-level inheritance, not
same-screen pixel fidelity and not aesthetic approximation.

Derive applicability from the baseline's component/slot/state/region/icon/
anchor mappings and the screen's independently bound identities. Compare every
applicable clause with observed candidate computed values and exact-cover it as
`conforms`, `accepted_exception`, or `source_backed_not_applicable`. Never let a
screen contract or candidate establish its own fixed truth. Delegated design
may fill only genuinely ungoverned screen-specific composition and may not
replace, weaken, omit, or approximate a fixed clause.
...
Keep baseline context to the smallest exact set within the active runtime
profile ceiling of 16 files and about 600k estimated text tokens.
...
The current profile passes, including exact source/standard and target-clause
coverage owned by that gate; no known next-stage trap remains; no unauthorized
