...
from custometry_api.installation import InstallationStatus
...
def create_app(
    *,
    ...
    async def no_store_health_responses(
        ...
        if request.url.path.startswith(("/health/", "/version", "/installation/status")):
    ...
    @app.get(
        ...
        responses={503: {"model": InstallationStatus}},
        tags=["installation"],
    )
    def installation_status() -> InstallationStatus | JSONResponse:
        payload = PostgreSQLReadinessProbe(runtime_settings).inspect()
        if payload.code is not None:
            return JSONResponse(
                status_code=503, content=payload.model_dump(exclude_none=True, by_alias=True)
            )
        return payload
    ...
    app.mount("/semantic", create_semantic_app(runtime_settings), name="semantic")
    ...
    _ = (no_store_health_responses, live, ready, version, installation_status)
    return app
