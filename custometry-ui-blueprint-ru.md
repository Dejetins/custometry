...
Уточнение владельца от **2026-09-20**, редакция `2026-09-20.1` (METRIC-030…035, REPORT-018/019):

- График и таблица поддерживают день / неделю / месяц / квартал / полугодие / год; неполные интервалы видимы, расчёт и сравнение выполняются сервером.
- Настройки отчёта и его целей меняет **только создатель отчёта**. Роль администратора или право редактирования другого объекта этого не меняет. Читатель использует разрешённые фильтры и личное отображение, но не меняет состав, формулы, target и goal policy.
- В режиме настройки создатель выбирает достижение минимума или соблюдение максимума, конкретное значение или относительное изменение к заданной базе, разовую или повторяющуюся цель. Во время использования отчёта эти решения не переключаются.
- Цель сохраняет собственные условия при смене фильтров просмотра; изменения создателем оставляют историю. Для upper bound интерфейс отличает расход лимита от выполнения lower-bound цели.
- Пересчёт факта — явная команда. Итог фиксируется после окончания периода при достаточной полноте данных; исправление закрытого итога создаёт новый расчёт с историей. Run rate включается отдельно, показывает допущения и не заменяет факт или итог.
- Общие наборы доступны текущим читателям отчёта и данных. Личные параметры сохраняются до явного применения новой общей версии или сброса; текущая проверка доступа обязательна.

Подробные решения и границы этапов: [WS-003 2.0.0](docs/architecture/planning/directions/DIR-004/workstreams/WS-003.md). Принятая композиция сохраняется; демонстрационный прототип не доказывает реализацию этих правил.

Уточнение владельца от **2026-09-20**, редакция `2026-09-20.2` (METRIC-030): в базовых настройках компании/workspace должен настраиваться финансовый календарь. Минимальная форма показывает начало финансового года, правило подписи FY, примеры дат кварталов/полугодий/года и версию; запись требует административного права, а не creator-права на отчёт. В отчёте видны календарная/финансовая основа и точные даты. Изменение общего календаря не меняет сохранённый результат; создатель явно принимает новую версию через Apply/Save. Конкретный месячный профиль, API и проверки описаны в [MS-004 0.2.0](docs/architecture/planning/milestones/MS-004/plan.md); это план, не реализованная форма.
...
### 11.6. Обновление документа, готовые снимки и состояние

Согласование владельца от 2026-09-06: REPORT-REFRESH-001…007,
REPORT-PERF-001…002, OPS-009…010; распределение ответственности и proof —
[контракт обновления и восстановления](./docs/contracts/report-refresh-serving-recovery-contract.md).
Настройка доступна на каждом поддерживаемом dashboard, workbook-report и
research document через общий document UI; фиксированный historical snapshot
остаётся неизменяемым.

- Автор с необходимыми правами задаёт после поступления данных, по расписанию
  либо вручную; для расписания — период/время/IANA timezone и включение/паузу.
  Видны следующий срок, последняя проверка, успешное обновление и дата данных.
  Новые опубликованные рабочие отчёты/dashboards по умолчанию используют
  after_ingestion, research и зафиксированные публикации — manual/pinned.
  Старые документы не переводятся автоматически на новый default.
- Если срок наступил, но новые проверенные данные ещё не готовы, показывается
  «Ожидает новых данных»; после их публикации выполняется ожидающее обновление.
  Неизменные данные не создают фиктивное «Обновлён».
- В header, списке документов и настройке обновления используется одна
  server-authoritative projection. «Готов», «Запланировано», «Ожидает новых
  данных», «В очереди», «Обновляется», «Не удалось обновить» и «Обновление
  заблокировано» имеют EN/RU эквиваленты и безопасное объяснение.
- Доступность/целостность текущего результата, его свежесть и попытка обновления
  различаются: «Готов · данные на … · не удалось обновить» допустимо.
  «Результат повреждён» показывается только при подтверждённой corruption;
  временная недоступность storage имеет отдельное «Результат недоступен».
  «Обновлён …» указывает время успешной публикации, но не заменяет data-as-of.
  Состояние и разрешённое действие передаются текстом, не только цветом.
- Во время чтения данные не подменяются автоматически. «Доступна новая версия»
  и действие «Применить обновление» переключают целый снимок с сохранением
  совместимых страницы, фильтров и положения; несовместимости объясняются.
  Новый обычный open получает current snapshot, exact deep link — закреплённый.
  При revoke недоступные значения не удерживаются ради last-good UX.
- Готовый отчёт загружает bounded results активной страницы. Просмотр и переход
  между страницами не запускают повторный эквивалентный анализ. Новый фильтр
  может требовать нового расчёта, что раскрывается через существующий preflight.
  Цель — субъективно моментальный ready open при профиле до 50 активных аналитиков
  и 100 читателей одновременно; численные budgets ещё предстоит согласовать.
- В существующей Storage, retention, backup and restore surface администратор
  задаёт ежедневный вечерний backup либо другой разрешённый график, timezone,
  внешнюю относительно primary server цель и retention. Видны next/last success,
  age/failure и последний проверенный restore; незаданные destination/time
  не показываются как работающая защита. Статус расписания не гарантирует RPO/RTO.
