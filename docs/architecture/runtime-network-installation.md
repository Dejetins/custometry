...
## Installed HTTPS and operational status — MS-002-S02

[MS-002 1.0.0](planning/milestones/MS-002/plan.md) fixes this implementation;
[S02 evidence](../../.codex/delivery/evidence/MS-002/MS-002-S02/report.md) records
its observed boundary. The S01 section below is retained as historical increment
scope. Current installer commands additionally require validated TLS before a
fresh installation and expose `rotate-tls`.

Supply `--certificate <chain.pem> --private-key <key.pem> --trust-root <ca.pem>`
with `install`, or with `resume` to complete the retained S01 installation.
Interactive fresh installation prompts for missing file references. Issuance and
CA custody remain external: use the selected mkcert for internal trials, or
provide trusted PEM inputs. No CA private key is copied into an installation.
Certificate/key pairing, IP SAN, current validity and chain trust are checked
with OpenSSL; protected immutable copies live under a private TLS revision.
Only key/chain leaves are mounted read-only into Edge, at `/run/tls/`.
Trust roots remain with the operator-side verifier. Credentials known to be
compromised must be withdrawn by their external custodian; this is not an online
revocation or automatic-renewal service.

The generated installed Edge configuration listens on 8443 only and uses the
fixed `web:8080` upstream. It permits GET `/`, `/assets/`, `/docs/`, and exact
`/api/health/live`, `/api/health/ready`, `/api/version`,
`/api/installation/status`. Other routes return 403 and other methods return 405.
...
IPv4. An omitted port is allocated by Docker to a temporary owned Edge image
process, inspected and persisted; the real Edge bind then either acquires that
same port or fails. There is no silent reselection on contention or retry.
Only Edge publishes ingress, with separate `edge_to_web` / `web_to_api` networks.
This does not qualify production Edge egress denial under SEC-018.

`rotate-tls --root <root> --certificate <new-chain.pem> --private-key <new-key.pem>
--trust-root <ca.pem>` creates a protected candidate and runs actual Nginx config
validation before promotion. After activation, the installer compares the
trusted peer certificate with the selected leaf and verifies the real API status.
Rejected PEM/config candidates preserve the active configuration. Post-activation
failure stops Edge and restores only a currently valid prior revision; expired
material requires forward repair. HTTP is never a recovery transport. Candidate
files are retained privately for bounded diagnosis; no volume or global cleanup
is performed. A crash between state/config writes is reported as `CONFIG_CHANGED`
and requires operator reconciliation of retained owned revisions, not blind
acceptance of mismatched bytes.

The existing `/health/live`, `/health/ready`, `/version` DTOs keep their shapes.
Readiness now requires a real DB query, exactly one Alembic row equal to
`0009_notifications`, and create/write/fsync/unlink at the canonical artifact
root. It never creates the root or applies migrations. A failed DB, wrong/missing
head or unwritable mount returns not-ready. Installation status returns only:

```json
{"schema_version":"custometry-installation-status/v1","state":"ready_for_bootstrap","version":"<version>","components":{"database":"ready","schema":"ready","storage":"ready"},"next_action":"bootstrap_not_available"}
```

Any failed component uses `not_ready`, HTTP 503 and one safe `code`:
`DATABASE_UNAVAILABLE`, `SCHEMA_INCOMPATIBLE`, or `STORAGE_UNAVAILABLE`, in that
priority order. Both successful and failed status responses use `Cache-Control:
no-store`. No Identity table, host path, SQL, secret or account claim is returned.
The [OpenAPI](../../packages/contracts/openapi/foundation.openapi.json) and
[generated client](../../packages/contracts/src/foundation-client.ts) are the
consumer contract for S03.

Operator `status` independently checks owned config hashes, protected TLS files,
actual image/process identities, API environment, artifact and TLS mounts,
network membership, exact published mapping, trusted peer and API payload.
Failure never returns a ready URL. `stop` retains identity, port, secrets and
volumes. Recover DB availability with `resume`; incompatible schema requires
forward repair; restore canonical writable storage without world-writable modes.
`TLS_INVALID`, `TLS_CHANGED`, `TLS_ACTIVE_MISMATCH`, `CONFIG_CHANGED`,
`CONFIG_RUNTIME_MISMATCH`, `MOUNT_MISMATCH`, `NETWORK_MISMATCH` and
`PORT_MAPPING_MISMATCH` identify failed checks without dumping runtime details.

Compatibility: stronger readiness and restricted installed HTTPS intentionally
change prior weak/unrestricted guarantees; the status DTO is additive. Domain
schema and old delivery bytes are unchanged. S02 uses a locally built API candidate
for actual runtime proof. S04 still owns a newly versioned distributable bundle,
Mac/Linux target matrix and second-computer LAN proof; the old MS-001 API cannot
satisfy this new status contract. First-run UI and bootstrap remain S03/C04 work.

## Persistent installer increment — MS-002-S01

[MS-002 1.0.0](planning/milestones/MS-002/plan.md) selects this bounded increment.
The [S01 report](../../.codex/delivery/evidence/MS-002/MS-002-S01/report.md)
and [journal](../../.codex/delivery/ledgers/MS-002.md) record actual completion and
remaining target setup. This section describes the new implementation, separately
from the retained MS-001 supply and the target end-to-end contract below.

The operator entry is [install.sh](../../deploy/compose/install.sh), named
`custometry-install` in help. Set `CUSTOMETRY_INSTALL_PYTHON` to an explicit Python
3.12 executable. It runs with `-I` and uses only the standard library. The producer
can run `python3 deploy/compose/package-installer.py --output <new-directory>` to
copy the launcher and two Python modules into a standalone toolkit, with a SHA-256
inventory for its separately trusted handoff. The consumer needs no checkout,
pip, uv, Node, source build or registry transfer. The existing developer bootstrap
is a separate entry and retains its HTTP behavior.

```sh
CUSTOMETRY_INSTALL_PYTHON=/absolute/path/to/python3.12 \
  /absolute/toolkit/deploy/compose/install.sh install \
  --root /absolute/new/private/root \
  --source /absolute/protected/supply --trust /absolute/independent/inventory.json \
  --port 18443 --demo
```

Omit `--demo` for core only. On a terminal, missing root/supply/inventory inputs
are prompted. `--lan --bind <interface-IPv4>` records the explicit LAN selection;
loopback is the default. `--certificate`, `--private-key` and `--trust-root` record
external TLS references for S02; S01 does not validate or activate HTTPS. Port zero
means pending engine allocation in S02. Explicit ports are checked before fresh
installation and retained with identity. No service publishes a port in S01.

`installation.json` uses `custometry-installation/v1`, with random installation
ID, canonical root, exact Docker context/engine/endpoint/platform, trusted supply
inventory, selected origin/port, optional demo, TLS references, image subjects,
volume/network ownership, resource accounting, last completed step and safe
failure code. The JSON is data, never sourced by a shell. Root and private
subdirectories are 0700; atomic state/config files are 0600. Password leaves use
64 lowercase hex characters and 0444 under the private parent for the existing
non-root bind-mount contract. The artifact root mounts at
`/var/lib/custometry/artifacts` and is assigned to the API image's runtime UID.

`install`, `resume`, `stop` and `status` acquire the same POSIX lock. Unknown
roots, schema, symlinked paths, foreign resource labels, changed Compose bytes,
engine/context mismatch and unsupported image stores fail closed. Acquisition
copies to a new private attempt, checks independent hashes, executes the retained
verified reader, then atomically exposes the candidate. Interrupted attempts
remain inside the owned root for diagnosis. No automatic deletion, adoption,
global prune, image removal or volume removal is implemented.

The DB's final TCP server must respond before the one-shot job runs. A fresh DB
is migrated to `0009_notifications`; an already supported head is inspected and
preserved without rerunning a completed migration. Other heads require forward
repair. A live migration left after a client timeout returns `MIGRATION_RUNNING`.
Command failures/timeouts persist `failed` and the last completed step. The job
has a ten-minute deadline, DB polls use two-second intervals with a 120-second
deadline, and individual probes are bounded. S01 stops at `migrated`: API/Web/Edge
startup, stronger status, TLS rotation and the engine-selected HTTPS mapping
belong to S02. `ready` remains false and URL remains null even after migration.

Use the same launcher with `status --root <root>`, `stop --root <root>` or
`resume --root <root>`. Stop retains volumes, secrets, origin and acquired bytes.
A repeated install resumes the same identity. Runtime ownership checks inspect
exact Compose and installation labels before mutation. Preserve the source
MS-001 toolkit/bundle and original trusted inventory unchanged.

| Safe code | Operator recovery |
|---|---|
| `PYTHON_312_REQUIRED` | Select the approved Python 3.12 executable explicitly |
| `CONTAINERD_STORE_REQUIRED` / `REMOTE_ENGINE_UNSUPPORTED` | Use the selected local native containerd engine; do not switch engine/store implicitly |
| `ENGINE_CONTEXT_MISMATCH` / `ENGINE_IDENTITY_MISMATCH` | Restore the original context/engine; never adopt another engine's state |
| `FOREIGN_ROOT` / `FOREIGN_RESOURCE` / `PATH_OWNERSHIP` | Select a new private root or resolve exact resource ownership; no automatic adoption |
| `INSTALLATION_LOCKED` | Wait for the owning command; never delete a live lock |
| `TRUSTED_HASH_MISMATCH` | Restore the exact trusted supply bytes before resume |
| `CONFIG_CHANGED` / `SECRET_FORMAT` / `SECRET_MISSING` | Restore the owned valid files; never regenerate secrets as a recovery shortcut |
| `INSUFFICIENT_DISK` / `OWNED_DATA_LIMIT` | Account for retained attempts, native unpacked images and artifact temporary data; free only independently authorized resources |
| `PORT_UNAVAILABLE` / `SAVED_ORIGIN_MISMATCH` | Resolve the selected port conflict; do not silently change a persisted origin |
| `COMMAND_TIMEOUT` / `MIGRATION_RUNNING` | Inspect the recorded owned job and DB head before retry; a timeout is not success |
| `SCHEMA_FORWARD_REPAIR_REQUIRED` / `MIGRATION_HEAD` | Inspect actual Alembic state; no downgrade, domain schema change or volume reset |
| `HTTPS_NOT_IMPLEMENTED` | Expected S01 boundary; S02 supplies TLS and installed readiness |

The configured service memory sum stays below 6 GiB and aggregate CPU caps stay
within four available cores. Disk preflight reserves the 25 GiB data ceiling plus
acquisition/expansion space, records native unpacked sizes and retained attempts,
and rejects oversized supply. This is bounded preflight/accounting, not a live
filesystem quota or a peak-resource measurement. The selected UTM guest has
4 CPU, 8 GiB RAM and a 40 GiB sparse disk; OS overhead is separate. Actual guest,
CA trust and M5 observations belong to S01 evidence, not this target description.
