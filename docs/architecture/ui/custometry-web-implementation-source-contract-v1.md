...
Ticket frontmatter is the status authority. A coordination graph records
dependencies and disjoint path ownership, not a second status register.
No prompt pack, stage ledger, generated-prompt inventory, or duplicated
certification layer is part of this route. Complete product coverage remains
the goal; it does not require rebuilding a G-program.
...
Every Web implementation ticket names exact screen or surface IDs, requirement
...
`tests/e2e/<ticket-slice>/` path. It uses Playwright's `webServer` lifecycle to
start host Vite on an explicit loopback port, wait for readiness, fail on startup
error, and clean up. It discovers only ticket-owned specs and declares desktop
...
Add remaining cohesive product slices or conformance work as ordinary tickets.
