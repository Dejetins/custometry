...
describe("canonical route registry", () => {
  it("contains the 91 normative route-level UI entries", () => {
    expect(routeRegistry.routes).toHaveLength(91);
    expect(new Set(routeRegistry.routes.map((route) => route.id)).size).toBe(91);
  });
